...
export default function Products({ params }: { params: { id: string } }) {
  ...
  const handleAddToCart = async () => {
    try {
      ...
      searchParams.set("callbackUrl", `${window.location.hostname}/product/detail/clsj0yzk000011tvxnwoqbz1u`)
    ...
    <main className="flex flex-col w-full min-h-screen items-center pb-8">
      <div className="w-content flex pt-5 gap-12">
        <div className="border p-2 rounded-xl">
          <div className="w-[376px] h-[376px] relative">{productDetails?.data.img ? <Image src={productDetails?.data.img} layout="fill" alt="" objectFit="contain" /> : null}</div>
        </div>
        <div className="flex flex-col gap-4">
          <div className="text-leaf font-semibold">{productDetails?.data.category}</div>
          ...
          <div className="text-4xl font-semibold">Rp {formatNumber(productDetails?.data.price || 0)} / kg</div>
          <div className="text-gray-400">{productDetails?.data.description}</div>
          <div className="flex gap-4 items-center">
            <CommonStepper count={itemCount} onChange={(count) => setItemCount(count)} />
            <Button className={cn("py-1 px-4 bg-leaf text-white leading-4", hover.shadow)} onClick={handleAddToCart}>
              <IconCart className="w-5 h-5 mr-2" />
              Masukkan Keranjang
            </Button>
            <Button className={cn("py-1 px-4 bg-carrot text-white leading-4", hover.shadow)} onClick={handleAddToCart}>
      ...
      <div className="w-content">
        ...
        <ProductShowcase gridConfig={"grid-cols-4"} products={recommendedProducts?.data?.data.slice(0, 4) || []} />
      </div>
    </main>
  )
}
